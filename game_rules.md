...
#### **6.7 效果结算优先级 (Effect Resolution Priority)**

为解决多个效果同时生效时的冲突，所有效果的结算遵循以下优先级顺序。在同一优先级内，**禁止性/无效化效果**（如“无效”、“不能”、“不增加”）的优先级高于**修正性/增加性效果**（如“+5”、“翻倍”、“-2”）。

1.  **天时规则 (Time Phase Rules):** 由每轮“流年干支”决定的基础生克位效果。
2.  **星象牌效果 (Celestial Card Effects):** 当前生效的全局宏观规则。
3.  **全局卦象效果 (Global Hexagram Effects):** 由特定基础牌（如《泰》、《否》）发动的、影响所有玩家的全局效果。
4.  **奇门八门效果 (Qi Men Gate Effects):** 玩家在移动阶段结束后触发的区域即时效果。
5.  **玩家卦象解读 (Player Hexagram Interpretation):** 玩家在解读阶段翻开自己的牌并执行的效果。

> **示例解析：**
>
> 场景：一个五行属“土”的【天部】区域。本轮天干为“甲”（木，克土），地支为“午”（火，生土）。
> 1.  **天时结算：**
>     *   因“午”生“土”，此区域获得“生位”状态，奖励+5金币。
>     *   因“甲”克“土”，此区域获得“克位”状态，“本轮不会累积金币”。
> 2.  **优先级判定：** 两个效果都属于最高优先级“天时规则”。在同一优先级内，“禁止性”效果（不累积）高于“增加性”效果（+5金币）。
> 3.  **最终结果：** 该区域本轮**不累积**任何金币。
