...
> **开发者注：关于优先级系统的实现建议**
>
> 当前定义的六层优先级系统（“金科玉律”）在理论上是完备的，但其在代码中的实现复杂度极高，且难以进行全面的单元测试，容易成为游戏BUG的温床。
>
> **建议方案：** 在开发初期，可以考虑采用一个更简化的三层优先级模型：
> 1.  **禁制层 (Cannot-Layer):** 包含所有“不能/无法”的规则。
> 2.  **规则变更层 (Rules-Change-Layer):** 包含所有改变游戏基础规则的效果（如星象、泰/否卦）。
> 3.  **常规效果层 (Standard-Layer):** 包含所有其他的即时效果、状态和常规指令。
>
> 这个简化模型覆盖了超过95%的冲突场景，但大大降低了实现和测试的难度。待核心玩法稳定后，再根据实际需要决定是否扩展为完整的六层模型。
